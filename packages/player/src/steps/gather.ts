...
export const gather = Effect.fn("gather")(function* () {
	const config = yield* Config;
	const client = yield* ArtifactClient;

	const gatherResponse = yield* Effect.promise(() =>
	...
	return yield* new ArtifactError({
		message:
			gatherResponse.error?.error?.message ??
			"Artifacts API gathering request failed",
	});
});
